...
export interface OxContentOptions {
  ...
  /**
   * Glob patterns for files to exclude from SSG processing.
   *
   * Patterns are matched against paths relative to `srcDir` using POSIX-style
   * separators on every platform. Matched files are skipped during page
   * generation, navigation building, and search indexing in both the production
   * build and the dev server. Useful for excluding templates, drafts, or
   * partials that should not be served as standalone pages.
   *
   * @default []
   */
  exclude?: string[];
